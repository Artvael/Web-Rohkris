import type { Song } from '../types';

export const SONGS_DATA: Song[] = [
  {
    id: 's-give-thanks',
    title: 'Give Thanks (Bersyukur)',
    artist: 'Don Moen / Henry Smith',
    key: 'F / G',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Give thanks with a grateful heart
Give thanks to the Holy One
Give thanks because He's given Jesus Christ, His Son

And now let the weak say, "I am strong"
Let the poor say, "I am rich
Because of what the Lord has done for us"

Reff (Versi Indonesia):
Bersyukur dengan hati tulus
Bersyukur pada Yang Kudus
Bersyukur kar'na Dia b'rikan Yesus Kristus Putra-Nya

Kini yang lemah berkata, "Ku kuat"
Yang miskin berkata, "Ku kaya"
Kar'na apa yang t'lah Tuhan buat bagi kita
Bersyukur!`,
    chordsSnippet: 'Intro: F - C/E - Dm - Am - Bb - F/A - Eb - C',
  },
  {
    id: 's-1',
    title: 'KebaikanMu Menuntunku (Goodness of God)',
    artist: 'Bethel Music / NDC Worship',
    key: 'G / A',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Kucinta Kau Tuhan, kasih-Mu tak pernah gagal
Setiap hari 'ku dalam tangan-Mu
Dari saat 'ku bangun, hingga kuberbaring
'Ku mau bernyanyi tentang kebaikan-Mu

Reff:
Sepanjang hidupku Kau setia
Sepanjang hidupku Kau sangat baik
Dengan segenap nafasku yang ada
'Ku mau bernyanyi tentang kebaikan-Mu

Bridge:
Kebaikan-Mu mengejarku, selalu mengejarku
Kebaikan-Mu mengejarku, selalu mengejarku
Kuserahkan hidupku, kuserahkan diriku
Kebaikan-Mu mengejarku, selalu mengejarku`,
    chordsSnippet: 'Intro: G - C - G - D/F# - Em - C - D - G',
  },
  {
    id: 's-seperti-rusa',
    title: 'Seperti Rusa Rindu Sungai-Mu',
    artist: 'Welyar Kauntu / Symphony Music',
    key: 'C / D',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Seperti rusa rindu sungai-Mu
Jiwaku rindu Engkau
Kaulah Tuhan hasrat hatiku
Kurindu menyembah-Mu

Reff:
Kaulah kekuatan dan perisaiku
Kepada-Mu rohku berserah
Kaulah Tuhan hasrat hatiku
Kurindu menyembah-Mu

Yesus, Yesus, Kau segalanya bagiku
Yesus, Yesus, Kau segalanya bagiku`,
    chordsSnippet: 'Intro: C - G/B - Am - C/G - F - G - C',
  },
  {
    id: 's-way-maker',
    title: 'Way Maker',
    artist: 'Sinach / Leeland / True Worshippers',
    key: 'E / G',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `You are here, moving in our midst
I worship You, I worship You
You are here, working in this place
I worship You, I worship You

Reff:
(You are) Way maker, miracle worker, promise keeper
Light in the darkness, my God, that is who You are

(Versi Indonesia):
Kau hadir di sini, bekerja di tengah kami
Kusembah Kau, kusembah Kau
Kau pembuat jalan, pembuat mukjizat, penggenap janji
Terang dalam g'lap, Tuhanku, itulah Engkau!`,
    chordsSnippet: 'Intro: E - B - C#m - A / G - D - Em - C',
  },
  {
    id: 's-10000-reasons',
    title: '10,000 Reasons (Bless the Lord)',
    artist: 'Matt Redman',
    key: 'G / A',
    tempo: 'Medium',
    category: 'Penyembahan',
    lyrics: `Chorus:
Bless the Lord, O my soul, O my soul
Worship His holy name
Sing like never before, O my soul
I'll worship Your holy name

Verse 1:
The sun comes up, it's a new day dawning
It's time to sing Your song again
Whatever may pass, and whatever lies before me
Let me be singing when the evening comes

Verse 2:
You're rich in love, and You're slow to anger
Your name is great, and Your heart is kind
For all Your goodness I will keep on singing
Ten thousand reasons for my heart to find`,
    chordsSnippet: 'Intro: C - G - D/F# - Em - C - G - Dsus4 - D',
  },
  {
    id: 's-karya-terbesar',
    title: 'Karya Terbesar',
    artist: 'Sari Simorangkir',
    key: 'G / A',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Kasih yang terindah, hati yang mulia
Hanya kutemukan di dalam-Mu Yesusku
Pujian dari hatiku selalu di setiap waktuku
Tiada pernah berubah kasih-Mu

Reff:
Karya terbesar dalam hidupku
Pengorbanan-Mu yang s'lamatkanku
Engkaulah harta yang tak ternilai
Yang kumiliki dan kuhargai
Yesus Engkau kukagumi`,
    chordsSnippet: 'Intro: G - D/F# - Em - C - Am - D',
  },
  {
    id: 's-janji-fajar',
    title: "Janji-Mu S'perti Fajar",
    artist: 'Franky Sihombing',
    key: 'C / D',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Ketika kuhadapi kehidupan ini
Jalan mana yang harus kupilih
Kutahu sungguh Engkau Bapa yang setia
Pasti s'lalu sediakan yang terbaik bagiku

Reff:
Anugerah-Mu tercurah bagiku
Menopang langkah hidupku
Janji-Mu s'perti fajar pagi hari
Dan tiada pernah terlambat bersinar
Cinta-Mu s'perti sungai yang mengalir
Dan kutahu betapa dalam kasih-Mu`,
    chordsSnippet: 'Intro: C - Em - F - G - C',
  },
  {
    id: 's-sentuh-hatiku',
    title: 'Sentuh Hatiku',
    artist: 'Jason Irwanto',
    key: 'G / A',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Betapa kukasihani hidupku yang hampa
Tanpa kasih-Mu Tuhan yang nyata
Sentuh hatiku ubah hidupku
Jadikan aku bejana-Mu

Reff:
Sentuh hatiku, ubah hidupku
Menjadi indah seturut firman-Mu
Ajarku taat, s'lalu setia
Menuruti seluruh kehendak-Mu`,
    chordsSnippet: 'Intro: G - D/F# - Em - C - D - G',
  },
  {
    id: 's-2',
    title: "S'bab Dia Hidup (Because He Lives)",
    artist: 'Hymn / JPCC Worship',
    key: 'C / D',
    tempo: 'Medium',
    category: 'Pujian',
    lyrics: `Anak Allah Yesus nama-Nya
Menyembuhkan, menyucikan
Mati di salib tebus dosaku
Kubur kosong membuktikan Dia hidup

Reff:
S'bab Dia hidup ada hari esok
S'bab Dia hidup 'ku tak gentar
Karena 'ku tahu Dia pegang masa depanku
Hidup jadi berarti s'bab Dia hidup`,
    chordsSnippet: 'Intro: C - F - C - G',
  },
  {
    id: 's-3',
    title: "Besar Anug'rahMu (Great is Your Grace)",
    artist: 'JPCC Worship',
    key: 'E / F',
    tempo: 'Medium',
    category: 'Penyembahan',
    lyrics: `Ku ada sebagaimana ku ada
Berdiri menghadap takhta-Mu Bapa
Semua kar'na anug'rah-Mu
Yang t'lah s'lamatkanku

Reff:
Besar anug'rah-Mu, melimpah kasih-Mu
Semakin nyata dengan perbuatanku
Yesus Kau perlindunganku, pengharapanku
Kusembah Kau Yesus, selamanya`,
    chordsSnippet: 'Intro: E - B/D# - C#m - A',
  },
  {
    id: 's-4',
    title: 'Nyanyi Bagi Dia (Praise the Lord)',
    artist: 'GMS Worship',
    key: 'D / E',
    tempo: 'Upbeat / Praise',
    category: 'Pujian',
    lyrics: `Angkat suara puji Dia
Raja s'gala raja yang perkasa
Bumi bersorak, langit berguncang
Kemuliaan-Nya nyata di sini

Reff:
Haleluya, Haleluya
Pujilah Tuhan yang berkuasa
Haleluya, Haleluya
Kemenangan ada bagi kita!`,
    chordsSnippet: 'Intro: D - G - Bm - A (Upbeat Drum)',
  },
  {
    id: 's-kaulah-harapan',
    title: 'Kaulah Harapan',
    artist: 'Sari Simorangkir',
    key: 'D / E',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Bukan dengan kekuatanku
Kudapat jalani hidupku
Tanpa Tuhan yang di sampingku
Kutak mampu sendiri
Engkaulah kuatku, yang menopangku

Reff:
Kupandang wajah-Mu dan berseru
Pertolonganku datang dari-Mu
Peganglah tanganku, jangan lepaskan
Kaulah harapan dalam hidupku`,
    chordsSnippet: 'Intro: D - A/C# - Bm - G - Em - A',
  },
  {
    id: 's-walau-ku-tak-dapat-melihat',
    title: 'Walau Ku Tak Dapat Melihat',
    artist: 'Franky Sihombing & True Worshippers',
    key: 'C / D',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Ku sadari jalanku tak s'lalu rata
Namun Kau ada di sampingku
Ku tahu tangan-Mu tak pernah lepas
Memegang setiap langkah hidupku

Reff:
Walau ku tak dapat melihat
Semua yang Kau rancangkan bagiku
Namun satu hal yang ku percaya
Kau jadikan s'galanya indah pada waktunya`,
    chordsSnippet: 'Intro: C - G/B - Am - F - Dm - G',
  },
  {
    id: 's-hidup-ini-kesempatan',
    title: 'Hidup Ini Adalah Kesempatan',
    artist: 'Pdt. Johny W. / Lagu Rohani',
    key: 'C / D',
    tempo: 'Slow Worship',
    category: 'Penyembahan',
    lyrics: `Hidup ini adalah kesempatan
Hidup ini untuk melayani Tuhan
Jangan sia-siakan waktu yang Tuhan b'ri
Hidup ini harus jadi berkat

Reff:
Oh Tuhan pakailah hidupku
Selagi aku masih kuat
Bila saatnya nanti ku tak berdaya lagi
Hidup ini sudah jadi berkat`,
    chordsSnippet: 'Intro: C - F - C - G - C',
  },
];
