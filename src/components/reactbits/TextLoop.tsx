...
const getViewDimensions = (shape: TextLoopShape) => {
  switch (shape) {
    case 'line':
      return { width: VIEW_W, height: 60, cy: 30 };
    case 'wave':
    case 'arch':
    case 'circle':
    case 'infinity':
    default:
      return { width: VIEW_W, height: 520, cy: 260 };
  }
};

const buildPath = (shape: TextLoopShape, curviness: number, ribbonWidth: number, cy: number): string => {
...
export const TextLoop: React.FC<TextLoopProps> = ({
  ...
  const { width: viewW, height: viewH, cy } = useMemo(() => getViewDimensions(shape), [shape]);

  const d = useMemo(
    () => path || buildPath(shape, curviness, ribbonWidth, cy),
    [path, shape, curviness, ribbonWidth, cy]
  );
