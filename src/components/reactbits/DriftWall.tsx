...
export const DriftWall: React.FC<DriftWallProps> = ({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner group relative">
        <img src={item.image} alt={item.reference ?? ''} loading="lazy" decoding="async" draggable={false} />
        <span className="drift-wall__overlay" aria-hidden="true" />
        {item.verse && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-3 text-center z-10 pointer-events-none">
            <p className="text-white text-[11px] font-bold drop-shadow-md leading-snug line-clamp-4 font-['Plus_Jakarta_Sans']">"{item.verse}"</p>
            <p className="text-amber-400 text-[10px] font-semibold mt-1 drop-shadow-md">{item.reference}</p>
          </div>
        )}
      </span>
    );
    ...
    return (
      <div key={id} tabIndex={0} role="button" aria-label={item.reference ?? 'tile'} {...commonProps}>
        {inner}
      </div>
    );
  };
